 ...
 * @param {Number} val
 */
export const checkNum = (val: any) => /^\d{1,}$/g.test(val);

/**
 *  判断是否是字母
 * @param {Number} val
 */
export const checkLetter = (val: any) => /^[a-zA-Z]+$/g.test(val);

/**
 * 判断是否是字母或数字
 * @param {Number || String} val  字符或数字
 */
export const checkNumOrLetter = (val: any) => /^[0-9a-zA-Z]*$/g.test(val);

/**
 * 判断是否是中文
 * @param {String} data  中文
 */
export const checkChinese = (val: any) => /^[\u4E00-\u9FA5]+$/g.test(val);
 ...
 * @param {String} key  属性
 */
export const localStorageGet = (key: any) => localStorage.getItem(key);

/**
 * localStorage 移除
 * @param {String} key  属性
 */
export const localStorageRemove = (key: any) => localStorage.removeItem(key);
